...
export function Hero({ product }: { product: Product }) {
  ...
  return <section className="hero-section legacy-hero iphone18-hero">
    ...
    <div className="legacy-hero-product">
      ...
      <Link href={`/iphone/${product.slug}`} data-event="product_view" data-product-id={product.id} aria-label={`Ver ${product.name}`}><Image src={getProductImage(heroColor.image)} alt={`Ilustración referencial de ${product.name} en color ${heroColor.name.toLowerCase()}`} width={1200} height={1200} priority /></Link>
