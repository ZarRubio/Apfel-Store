...
import { getProductImage, getDefaultProductColor } from '@/lib/productImage';

export function Hero({ product }: { product: Product }) {
  const heroColor = getDefaultProductColor(product);
  return <section className="hero-section legacy-hero iphone18-hero">
    ...
    <div className="legacy-hero-product">
      ...
      <Link href={`/iphone/${product.slug}`} data-event="product_view" data-product-id={product.id} aria-label={`Ver ${product.name} en color ${heroColor.name}`}><Image src={getProductImage(heroColor.image)} alt={`${product.name} en color ${heroColor.name.toLowerCase()}`} width={1200} height={1200} priority /></Link>
      <div className="legacy-product-caption"><span>{product.name} · Fotografía del producto</span><strong>Solicita tu reserva</strong></div>
    </div>
